...
(b)部區間中間點（mid）的算式溢位
困難點： 傳統課本或直覺的寫法是 int mid = (low + high) / 2;。當我們在測試大資料量（例如 $n = 5000$ 甚至更大）時，low + high 的相加數值極有可能會超過 C++ 中 int 的最大上限（2,147,483,647）。這會導致嚴重的整數溢位（Integer Overflow），算出來的中間點變成負數，進而引發陣列索引出錯、程式直接當掉（Segmentation Fault）。
