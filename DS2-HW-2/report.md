 ...
 **1.迭代版合併排序（Iterative Merge Sort）的邊界控制**
 ...
 迭代版合併排序（Iterative Merge Sort）的邊界控制投影片明確規定「Merge Sort 不能使用遞迴（Non-recursive）」。困難點：遞迴版的 Merge Sort 很好寫（直接二分遞迴即可），但改成用 for 迴圈由底向上（1 疊 2、2 疊 4、4 疊 8...）去合併時，最大的地獄在於「陣列長度不一定是 2 的冪次方」。當 $n=5000$ 或 $3000$ 時，迴圈切到最後一定會剩下孤兒碎塊。
 
 **困難點狀況及改善方法**

程式中必須精準使用 min(l + size - 1, n - 1) 來鎖死右邊邊界（r）和中間點（m），只要算錯 1 個字元，程式不是直接崩潰（Segmentation Fault）就是資料漏排序。
